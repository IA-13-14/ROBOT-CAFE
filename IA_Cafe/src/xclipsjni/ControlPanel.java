...
public class ControlPanel extends javax.swing.JFrame implements Observer {
	...
	public ControlPanel(ClipsModel model) {
		...
		agendaMonitor
				.setLocation(screenDim.width - agendaMonitor.getWidth(), 0);
		agendaMonitor.setVisible(true);

		factsMonitor = new PropertyMonitor("Facts");
		factsMonitor.setSize(propertyMonitorDim);
		factsMonitor.setLocation(screenDim.width - factsMonitor.getWidth(),
				agendaMonitor.getHeight());
	...
	private void setShortcut() {
		...
		runButton.getInputMap(runButton.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke(KeyEvent.VK_R, InputEvent.ALT_MASK),
				"run");
		runOneButton.getInputMap(runOneButton.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke(KeyEvent.VK_T, InputEvent.ALT_MASK),
				"run1");
		stepButton.getInputMap(stepButton.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.ALT_MASK),
				"step");
		loadDefaultFileButton.getInputMap(
				loadDefaultFileButton.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke(KeyEvent.VK_A, InputEvent.ALT_MASK),
				"start");
		resetButton.getInputMap(resetButton.WHEN_IN_FOCUSED_WINDOW).put(
	...
	private void initComponents() {
		...
		stepGoButton = new javax.swing.JCheckBox();
		...
		stepGoButton.setText("Step Go");
		stepGoButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				stepGoButtonActionPerformed(evt);
			}
		});
		...
		layout.setHorizontalGroup(layout.createParallelGroup(
				...
				controlPanel, javax.swing.GroupLayout.PREFERRED_SIZE,
				javax.swing.GroupLayout.DEFAULT_SIZE,
				javax.swing.GroupLayout.PREFERRED_SIZE));
	...
	private void visualizeDebugButtonActionPerformed(
	...
	private void visualizeFactsButtonActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_visualizeFactsButtonActionPerformed
		...
		if (visualizeFactsButton.isSelected())
			try {
				...
				System.out
						.println("The clips core is still not initialized - unable to get the facts list");
			}
	}// GEN-LAST:event_visualizeFactsButtonActionPerformed

	private void visualizeAgendaButtonActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_visualizeAgendaButtonActionPerformed
		...
		if (visualizeAgendaButton.isSelected())
			try {
				...
				System.out
						.println("The clips core is still not initialized - unable to check the agend content");
			}
	...
	private void loadDefaultFileButtonActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_loadDefaultFileButtonActionPerformed
		...
		String strategyFolder_name = CLPSelector.getSelectedItem().toString(); // La
																				// strategia
																				// scelta
		String envsFolder_name = envsSelector.getSelectedItem().toString(); // La
																			// cartella
																			// di
																			// env
																			// scelta
		model.startCore(strategyFolder_name, envsFolder_name); // Diciamo al
																// modello di
																// partire
		model.setMode("START");
		model.execute();
	...
	// java.util.logging.Logger.getLogger(ControlPanel.class.getName()).log(java.util.logging.Level.SEVERE,
	// null, ex);
