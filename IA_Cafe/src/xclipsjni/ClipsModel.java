 ...
 *         Varesano Marco, Busso Marco, Cotrino Roberto, Enrico Mensa, Matteo
 *         Madeddu, Davide Dell'Anna
 */
public abstract class ClipsModel extends Observable implements Runnable {
	...
	private boolean stepGo = false;
	private boolean runAndUpdate = false;

	public boolean isStepGo() {
		return stepGo;
	}

	public void setStepGo(boolean stepGo) {
		this.stepGo = stepGo;
	}

	public boolean isRunAndUpdate() {
		return runAndUpdate;
	}

	public void setRunAndUpdate(boolean runAndUpdate) {
		this.runAndUpdate = runAndUpdate;
	}
		...
		try {
			while (!hasDone()) {
				...
				if (executionMode == 1) {
					...
					long run_feedback = 1;
					/*
					 * Alla pressione del tasto START, il while entra una volta
					 * nel ciclo. Esegue una runOne(). Controlla che il fatto
					 * init-agent sia dichiarato (il mondo è pronto) Se è
					 * così, allora aggiorna l'interfaccia poiché il mondo è
					 * pronto. All'uscita dal metodo il thread principale si
					 * sospende in attesa di una nuova azione (pressione tasto)
					 * che farà la resume.
					 */
					while (run_feedback == 1) {
						run_feedback = core.runOne();
						String[] arrayInitAgent = { "done" };
						String[] initAgent = core.findFact("AGENT",
								"init-agent", "TRUE", arrayInitAgent);
						...
						// System.out.println("Eseguo una run one, cstep = " +
						// current_perc +" last_step = " + last_known_perc
						// +" e step ="+last_perc[0]);
					}
					this.suspend();
				}
				 ...
				 */
					...
					long run_feedback = 1;
					String[] arrayPercept = { "step" };
					String[] current = core.findFact("AGENT", "last-perc",
							"TRUE", arrayPercept);
					Integer actual;
					Integer prec;
					/*
					 * Se lo stato last-perc non esiste, allora lo stato attuale
					 * viene impostato a -1 Diversamente viene impostato allo
					 * stato di last-perc. Questo viene fatto per poter
					 * completare uno step ""sporcato"" dal click di una runOne
					 * e riallinearsi con le azioni necessarie ad arrivare alla
					 * prossima percezione del mondo (conseguente azione).
					 */
					if (current[0] == null)
						actual = -1;
					else
					...
					prec = actual;
					/*
					 * Lo stato precedente viene inizalizzato al valore dello
					 * stato attuale Fino a che lo stato precedente è uguale
					 * allo stato attuale, allora proseguo (devo arrivare alla
					 * prossima percezione, facendo una run.
					 */
					long start = System.currentTimeMillis();
					while (prec.equals(actual)) {
						run_feedback = core.runOne();
						current = core.findFact("AGENT", "last-perc", "TRUE",
								arrayPercept);
						if (current[0] == null)
							actual = -1;
						else
					...
					// System.out.println("Eseguo uno step, cstep = " + actual
					// +" last_step = " + prec);
					core.runOne();
					System.out.println();
					System.out.println("[CLIPS-EXEC-TIME]: "
							+ (System.currentTimeMillis() - start) + " ms.");
					/* Aggiorno l'interfaccia */
					action();
					this.setChanged();
					this.notifyObservers("actionDone");
					// QUESTA SUSPEND E' L'UNICA DIFFERENZA CON IL TASTO START
					// CHE INVECE NON SI SOSPENDE
					if (!stepGo)
						this.suspend();
				}
				/*
	 ...
	 *
	 * @param mode
	 *            ha 3 valori ammessi: RUN per eseguire i passi consecutivamente
	 *            senza interruzioni, RUNONE per eseguire un passo Clips alla
	 *            volta (in fase di debug), STEP per eseguire una exec alla
	 *            volta.
	 */
	 ...
	 * 
	 * @param strategyFolder_name
	 *            Nome della cartella in CLP che contiene tutti i file di CLIPS
	 *            (CLP/strategyFolder_name)
	 * @param envsFolder_name
	 *            Nome della cartella in CLP che contiene tutti i file relativi
	 *            all'environment (envs/envFolder_name)
	 */
	public void startCore(String strategyFolder_name, String envsFolder_name) {
		try {
			...
			DebugFrame
					.appendText("[Clips Environment created and ready to run]");
			/*
			 * effettua una reset di clips dopo aver caricato i file e carica le
			 * info iniziali dei file clips, per poi terminare la fase di setup
			 */
			core.reset();
			setup();
			this.setChanged();
			...
			Logger.getLogger(ClipsModel.class.getName()).log(Level.SEVERE,
					null, ex);
		}
	}
