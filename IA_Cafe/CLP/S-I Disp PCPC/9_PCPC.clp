;######## MODULE: Pre-calculated Paths Cost
; Description: This module calculates the cost of the path from given source position
;                to given destination position.

;/-----------------------------------------------------------------------------------/
;/*****                        Pre-calculated Paths Cost                        *****/
;/-----------------------------------------------------------------------------------/
...
(defrule calc-pcpc
    (declare (salience 50))
    (calculate-pcpc (source-direction ?src-d) (source-r ?src-r) (source-c ?src-c)
    ...
    =>
        ...
        (assert (start-path-planning (source-direction ?src-d) (source-r ?src-r) (source-c ?src-c)
                                 (dest-direction ?dst-d) (dest-r ?dst-r) (dest-c ?dst-c) (ignore-perceptions yes)))
        (focus PATH-PLANNER-HL)
)

;#### If Path doesn't exist (person blockin the way) ####
(defrule path-planner-result-no
    (declare (salience 60))   
    ?cf <- (calculate-pcpc (source-direction ?src-d) (source-r ?src-r) (source-c ?src-c)
                    (dest-direction ?dst-d) (dest-r ?dst-r) (dest-c ?dst-c))
    ...
    =>
        ...
        (assert (pcpc (source-direction ?src-d) (source-r ?src-r) (source-c ?src-c)
                    (dest-direction ?dst-d) (dest-r ?dst-r) (dest-c ?dst-c) (cost 99999)))
        (assert (pcpc (source-direction ?dst-d) (source-r ?dst-r) (source-c ?dst-c)
                    (dest-direction ?src-d) (dest-r ?src-r) (dest-c ?src-c) (cost 99999)))
        (retract ?f)
        (retract ?cf)
        ;(halt)
)
...
(defrule path-planner-result-yes
   ...
   ?cf <- (calculate-pcpc (source-direction ?src-d) (source-r ?src-r) (source-c ?src-c)
    ...
    =>
        ...
        (assert (pcpc (source-direction ?src-d) (source-r ?src-r) (source-c ?src-c)
                    (dest-direction ?dst-d) (dest-r ?dst-r) (dest-c ?dst-c) (cost ?cost)))
        (assert (pcpc (source-direction ?dst-d) (source-r ?dst-r) (source-c ?dst-c)
                    (dest-direction ?src-d) (dest-r ?src-r) (dest-c ?src-c) (cost ?cost)))
        (retract ?f)
        (retract ?cf)
        ;(halt)
)
