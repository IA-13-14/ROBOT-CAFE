...
(deftemplate access-cell
    ...
    (slot obj-r)
    (slot obj-c)
    (slot reachable (allowed-values yes no) (default yes)) ;becomes no when all paths to this cell are blocked by clients
)
...
(deftemplate desire
    ...
    (slot type (allowed-values deliver clean empty load move-away))
    (slot pos-r (default NA))
    (slot pos-c (default NA))
    (slot order (default NA))
    (slot possible (allowed-values yes no) (default yes))
)
...
(defrule BDI_loop_6_actions_planner_failed
    ...
    (status (step ?s) (time ?t))
    ?int <- (intentions_changed)
    =>
     	(assert (printGUI (time ?t) (step ?s) (source "AGENT") (verbosity 2) (text  "ACTIONS PLANNER FAILED, REPLANNING !")))
        (retract ?bdis)
        (modify ?int (changed yes))
        (assert (BDistatus BDI-4))
        (retract ?apr)
)
