...
(defrule BDI_loop_2_check_answer_order_delay
    ...
    ?desf <- (desire (type order) (order ?req-id) (penalty ?op))
    (status (step ?s))       
    (K-table (step ?s) (table ?otable) (state ~Clean)) 
    =>
        (retract ?bdis)
        (retract ?ansor)
        (modify ?desf (penalty (/ ?op 2))) ;update desire's penalty because order has been delayed
...
(defrule BDI_loop_6_actions_planner_failed
    ...
    =>
        ...
        (retract ?bdis)
        ;### Wait too expensive, Turn instead ###
